...
public class OrganizeText {
    ...
    static void init() {
        List<String> manNameListStr = Arrays.asList("南弦", "墨尘", "叶轩", "夜羽",
                "高玄", "博文", "霄鸣", "封禹", "付昊余", "睿凌", "岩朋", "冯承和", "江远", "阳澈", "无洛", "黎川",
    ...
    public static void main(String[] args) {
        ...
        init();
        String filename = "D:\\workspace\\MyTest\\src\\main\\resources\\原文.txt";
        String filename2 = "D:\\workspace\\MyTest\\src\\main\\resources\\原文-整理版.txt";

        //读取本地文件中的内容
        String content = readContent(filename);
        final String trim = content.replace(" ","");
        ...
        // 男性
        objectObjectHashMap.put("一冰", new String[]{"冰思"});
...
//        objectObjectHashMap.put("秋依枫", new String[]{"周信信"});
    ...
    private static String replaceDuoYin(String s1) {
        ...
        final String replace22 = replace19.replace("tm", "他喵");
        return replace22;
    }
    ...
    static String removePharseNum(String source) {
        ...
        final String 六六六 = 幺幺零.replace("666", "六六六");
        String content = 六六六;
    ...
    static String replaceSymbol(String text) {
        ...
        for (int i = 0; i < num + 1; i++) {
            ...
            for (int j = 0; j < 6; j++) {
                ...
                if(i==num){
                    substring1 = substring.substring(j * peiYinPageNum);
                }else {
                    substring1 = substring.substring(j * peiYinPageNum, (j + 1) * peiYinPageNum);
                }

                final int i1 = substring1.lastIndexOf("，");
                final String substring2 = substring1.substring(0, i1);

                if (j == 0) {
                    newString.append(substring2).append(",").append("\r\n");
                    lastStr = substring1.substring(i1+1);
                } else if (j == 5) {
                    newString.append(lastStr).append(substring2);
                    lastStr = substring1.substring(i1+1);
                    newString.append(lastStr);
                } else {
                    newString.append(lastStr).append(substring2).append(",").append("\r\n");
                    lastStr = substring1.substring(i1+1);
                }
            }
            stringBuffer.append(newString).append("\r\n").append(",").append("\r\n");
        }
