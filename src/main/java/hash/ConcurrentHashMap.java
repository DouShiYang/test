...
import hash.ConcurrentHashMap.Node;
...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, Serializable {
    ...
    /**
     * 最大可能的表容量。该值必须恰好为 1<<30 以保持在 Java 数组分配和两个表大小的幂的索引范围内，
     * 并且进一步需要，因为 32 位哈希字段的前两位用于控制目的
     */
    private static final int MAXIMUM_CAPACITY = 1 << 30;

    /**
     * 默认的初始表容量。 必须是 2 的幂（即，至少为 1）且最多为MAXIMUM_CAPACITY
     */
     ...
     * 此表的负载因子。
     * 在构造函数中覆盖此值仅影响初始表容量。
     * 通常不使用实际的浮点值——使用诸如n - (n >>> 2)表达式来表示相关的调整大小阈值会更简单
     */
    private static final float LOAD_FACTOR = 0.75f;

    /**
     * 使用树而不是列表的链表计数阈值。
     * 将元素添加到至少具有这么多节点的链表时，链表会转换为树。
     * 该值必须大于 2，并且应至少为 8，以与树移除中关于在收缩时转换回普通链表的假设相匹配
     */
    static final int TREEIFY_THRESHOLD = 8;

    /**
     * 在调整大小操作期间取消（拆分）链表的链表计数阈值。
    ...
    /**
     * 键值输入。
     * 此类永远不会作为用户可变的 Map.Entry（即一个支持 setValue；参见下面的 MapEntry）导出，但可用于批量任务中使用的只读遍历。
     * 具有负散列字段的 Node 子类是特殊的，包含空键和值（但永远不会导出）。
     * 否则，键和值永远不会为空。
     */
    ...
    /**
     * 散列 (XOR) 较高位的哈希值降低并强制最高位为 0。
     * 由于该表使用二次幂掩码，因此仅在当前掩码之上位有所不同的一组哈希值将始终发生冲突。
     * （众所周知的例子是在小表中保存连续整数的浮点键集。）所以我们应用了一种向下传播高位影响的变换。
    ...
    /**
     * table数组。 第一次插入时延迟初始化。 大小始终是 2 的幂。 由迭代器直接访问。
     */
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        ...
        //根据key的hashcode 获取散列值
        int hash = spread(key.hashCode());
        //定义链表长度
        int binCount = 0;
        for (Node<K, V>[] tab = table; ; ) {

            // 此节点可视为当前key落在数组上的位置上的头节点
            Node<K, V> f;
            int n; //数组的长度
            int i; //此数据应该位于数组第几个索引的位置
            int fh; //要插入的数据的 位于数组位置上的值的hash
            if (tab == null || (n = tab.length) == 0) {
                //如果当前table是空的 则进行初始化table
                tab = initTable();
            } else if ((f = tabAt(tab, i = (n - 1) & hash)) == null) {
                //根据当前i【i = 当前数组长度-1 与 当前的node的hashCode值】节点上判断是否存在元素，以此判断是否发生了hash碰撞
                //判读当前位置上是否有元素、如果没有 则cas自旋设置
                ...
                //将数组位置上的头节点f节点锁住
                synchronized (f) {
                    //取出i位置的节点 判断是否等于f 也就是判断f节点 是否是当前链表上的头节点
                    if (tabAt(tab, i) == f) {
                        //再判断数组中当前链表的头节点的hashcode值 是否大于0
                        if (fh >= 0) {
                            //定义当前链表的长度的为1
                            binCount = 1;
                            for (Node<K, V> e = f; ; ++binCount) {
                                K ek;
                                //如果当前的节点的key的hash值 与 value值 都等于要插入的数据信息 则将原来的数据返回 并保存最新的信息到当前位置上
                                ...
                                Node<K, V> pred = e;
                                //将下一个节点赋值给当前节点 直到原来的链表尾端时 将新增一个节点 挂载到最后位置
                                if ((e = e.next) == null) {
     ...
     * 数组下的红黑树实体对象
     * 此实体对象维护了treeNode的信息。
     * 这里维护了一个读写锁 迫使写线程【持有整个锁的】等待读线程完成
     */
    static final class TreeBin<K, V> extends Node<K, V> {
         ...
         * 我们不需要总顺序，只需要一致的插入规则来保持重新平衡之间的等效性。
         * 超出必要的打破平局会稍微简化测试。
         */
         ...
         * 这比典型的红黑删除代码更混乱，因为我们无法将内部节点的内容与叶后继交换，该后继由可独立于锁访问的“next”指针固定。因此，我们交换树链接。
         *
